...
#         Time complexity is O(n) but recopy min_sub each time a new sol
#         is found is slow, so store indexes of min_sub and copy string last,
#         following is (done in optimal sol)
#         """
#         freq_t = {}
#         for letter in t:
#             freq_t[letter] = 1 + freq_t.get(letter, 0)
    ...
    def minWindow(self, s, t):
        ...
        while end < len(s):
            ...
            if letter in freq_t and freq_s[letter] == freq_t[letter]:
                have += 1
            while have == need:
                if res_len == 0 or res_len > end - start + 1:
                    res_window = [start, end + 1]
                    res_len = end - start + 1
                freq_s[s[start]] -= 1
                if s[start] in freq_t and freq_s[s[start]] < freq_t[s[start]]:
                    have -= 1
                start += 1
            end += 1
        return s[res_window[0]: res_window[1]] if res_len > 0 else ""
